...
export const lensForProp = prop => ([whole => whole[prop], (whole, part) => ({ ...whole, [prop]: part })])
...
const useMemoizedLens = (state, setState) => useMemo(
  () => newLens(state, setState),
  [state]
)
...
export const useFractalForm = (initialValues) => {
  const initialState = { value: initialValues, error: {}, touched: {} }
...
export const useLensField = (useLens, fieldName, validate = () => null) => {
  const [{value, error, touched}, setField, useFieldLens] = useLens(
    form => ({
    ...
    (form, { value, error, touched }) => ({
      value: { ...form.value, [fieldName]: value },
      error: { ...form.error, [fieldName]: error },
      touched: { ...form.touched, [fieldName]: touched },
    })
  )

  const onChange = useCallback(event => {
    setField({ value: event.target.value, error: validate(value, event.target.value), touched })
  }, [touched])

  const onBlur = useCallback(() => setField({ value, error, touched: true }), [error, value])

  const field = { onChange, onBlur, value, error, touched }
